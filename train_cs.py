...
from dataloader import CS_Dataset
import models
import utils
...
def forward_FDA(rois, targets):
    ...
    for idx, roi in enumerate(rois):
        ...
        # constant > 0 is grl
        pixel_logits = model.classifier(roi_flatten,
                                        adaption=True,
                                        constant=-1 * args.mining_grl,
                                        pooling=False)
        pixel_scores = pixel_logits.softmax(-1)
        model.classifier.apply(enable_bn)

        sorted_value, sorted_index = pixel_scores[:, :-1].sort(-1,
                                                               descending=True)
    ...
    if len(unk_pixel_list) > 1:
        ...
        loss_mining_unk = p_xu_x * criterion_bce(
            mined_scores,
            torch.tensor([args.mining_th] * len(mined_scores)).cuda())
...
def forward_DCA(rois, all_layers=False, domain='source'):
    ...
    bs, c, h, w = rois.size()
    rois_flatten = rois.permute(0, 2, 3, 1).contiguous().view(-1,
                                                              c)  # bs, h, w ,c

    model.classifier.apply(fix_bn)
    if not all_layers:
        with torch.no_grad():
            scores = model.classifier(rois_flatten,
                                      pooling=False).softmax(-1).detach()
    else:
        scores, rois_flatten = model.classifier(rois_flatten,
                                                pooling=False,
                                                return_feat=True)
...
def train(epoch):
    ...
    for batch_idx in range(len(train_loader)):
        ...
        data_s, target_s = data_s.cuda(), target_s.long().cuda(
            non_blocking=True)
        data_t, target_t = data_t.cuda(), target_t.long().cuda(
            non_blocking=True)
        ...
        loss_align_s = forward_DCA(rois,
                                   all_layers=args.all_layer_adv,
                                   domain='source')
        ...
        loss_bp_t = criterion_bce(
            score_unk,
            torch.tensor([args.bp_th] * len(score_unk)).cuda())
        loss_dict_t.update(loss_bp_t=loss_bp_t)

        loss_align_t = forward_DCA(rois,
                                   all_layers=args.all_layer_adv,
                                   domain='target')
        ...
        if batch_idx % args.log_interval == 0:
            print_and_log(
                '[Epoch: {} {}/{} ({:.0f}%)] [Loss_s: {:.3f}, Loss_t: {:.3f}], [lr:{:.4f}] {}'
                .format(epoch, batch_idx * args.batch_size,
                        len(train_dataset.target_image),
                        100. * batch_idx / len(train_loader), loss_s.item(),
                        loss_t.item(), opts.param_groups[0]['lr'], {
                            **process_dict(loss_dict_s),
                            **process_dict(loss_dict_t)
                        }))
...
def warm_up_train(epoch):
    ...
    for batch_idx in range(len(train_loader)):
        ...
        if batch_idx % args.log_interval == 0:
            print_and_log(
                '[Warm Up: Epoch: {} {}/{} ({:.0f}%)] [Loss_s: {:.3f}], [lr:{:.4f}] {}'
                .format(epoch, batch_idx * args.batch_size,
                        len(train_dataset.target_image),
                        100. * batch_idx / len(train_loader), loss_s.item(),
...
def test(epoch):
    ...
    OS_star, unk = utils.cal_acc(true_y, pred_y, NUM_CLASSES, tf_writer, epoch,
                                 test_dataset.class_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-root', default='~/data/DA/CS/', help='dataset root')
    parser.add_argument('--partition', default='train', help='train or test')
    ...
    parser.add_argument('--num-works',
                        default=4,
                        type=int,
                        help='num_works for dataloader')
    parser.add_argument('--known_class',
                        type=float,
                        default=9,
                        metavar='TH',
                        help='known_class')
    ...
    parser.add_argument('--bp-th',
                        type=float,
                        default=0.5,
                        metavar='TH',
                        help='threshold (default: 0.5)')
    parser.add_argument('--bp-grl',
                        type=float,
                        default=0.5,
                        metavar='TH',
                        help='grl adversarial weight (default: 0.5)')
    parser.add_argument('--seed', default=42, type=int, help='save path')
    parser.add_argument('--warm_up_epoch',
                        default=10,
                        type=int,
                        help='source-domain pretraining')

    parser.add_argument('--topk',
                        default=3,
                        type=int,
                        help='select potential unk regions')
    parser.add_argument('--mining_th',
                        default=1.0,
                        type=float,
                        metavar='TH',
                        help='unk label')
    parser.add_argument('--mining_grl',
                        type=float,
                        default=0.2,
                        metavar='TH',
                        help='grad scaler (default: 0.2)')

    parser.add_argument('--all-layer-adv',
                        action='store_true',
                        default=False,
                        help='align all layers')
    parser.add_argument('--adv-grl',
                        type=float,
                        default=0.1,
                        metavar='TH',
                        help='grl adversarial weight (default: 0.1)')
    parser.add_argument('--save-path', default='./rerun/cs/', help='save path')
    ...
    train_dataset = CS_Dataset(root=args.data_root,
                                 partition='train',
                                 source=args.source,
                                 target=args.target)
    ...
    test_dataset = CS_Dataset(root=args.data_root,
                                partition='test',
                                source=args.source,
                                target=args.target)
    ...
    opts = torch.optim.SGD(model.parameters(),
                           args.lr,
    ...
    txt_file_1 = save_results_root + '/{}'.format(args.source + '_' +
                                                  args.target + '.txt')
    txt_file_2 = save_results_root + '/latex_{}'.format(args.source + '_' +
                                                        args.target + '.txt')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        value = 'Epoch: {}: OS: {}, OS*: {}, unk: {}, HOS: {} '.format(
            epoch, OS, OS_star, unk, HOS)
        logging.info(value)
        ...
        if HOS > HOS_bank:
            ...
            if not os.path.exists(dirs):
                os.makedirs(dirs)
            torch.save(
                model.state_dict(), dirs +
                f'{args.exp_name}_{epoch}_OS*{OS_star}_unk{unk}_HOS{HOS}.pt')
